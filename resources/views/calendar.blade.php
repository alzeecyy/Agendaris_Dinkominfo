            ...
            <div class="bg-slate-50/70 border border-[#d4d1f5]/60 rounded-3xl p-4 sm:p-5 shadow-xs shrink-0">
                <div class="flex items-center justify-between mb-4 border-b border-[#d4d1f5]/30 pb-2">
                    ...
                    <button type="button"
                        ...
                        class="p-1.5 hover:bg-[#8e88dd]/20 rounded-xl text-[#2e2552] transition-colors" title="Sebelumnya">
                    ...
                    <button type="button"
                        ...
                        class="p-1.5 hover:bg-[#8e88dd]/20 rounded-xl text-[#2e2552] transition-colors" title="Berikutnya">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div x-show="!showMonthPicker"
                    class="grid grid-cols-7 gap-1 text-center text-[10px] font-bold text-[#5a508f] mb-2">
                    <span>Sen</span><span>Sel</span><span>Rab</span><span>Kam</span><span>Jum</span><span
                        class="text-indigo-500 font-extrabold">Sab</span><span
                        class="text-rose-500 font-extrabold">Min</span>
                </div>
                ...
                <div x-show="showMonthPicker" x-cloak class="grid grid-cols-3 gap-2 text-center text-xs py-1">
                    <template
                        x-for="(mName, mIdx) in ['Jan', 'Feb', 'Mar', 'Apr', 'Mei', 'Jun', 'Jul', 'Agu', 'Sep', 'Okt', 'Nov', 'Des']"
                        :key="mIdx">
                        <button type="button"
    ...
    <div class="flex-1 min-w-0 bg-slate-50/70 border border-[#d4d1f5]/60 rounded-xl md:rounded-3xl p-3 sm:p-5 shadow-xs flex flex-col">
        ...
        <div class="flex-1 w-full flex flex-col overflow-hidden">
            <div class="flex-1 min-w-0 flex flex-col relative h-full overflow-x-auto overflow-y-hidden">
                <div class="min-w-[540px] sm:min-w-full flex-1 flex flex-col h-full min-h-[500px] sm:min-h-[560px]">
            ...
            <div class="min-h-[500px] sm:min-h-[560px] flex-1 grid grid-cols-8 relative z-10 select-none border-b border-[#d4d1f5]/40">
                @php
                    $labelTimes = [
        ...
        <!-- RIGHT PANEL: 7-Day Weekly Agenda Grid -->
        <div
            class="flex-1 min-w-0 bg-slate-50/70 border border-[#d4d1f5]/60 rounded-xl md:rounded-3xl p-3 sm:p-5 shadow-xs flex flex-col">
            <!-- Calendar Navigation Header -->
            <div
                class="flex items-center justify-between mb-3 sm:mb-4 gap-2 sm:gap-4 border-b border-[#d4d1f5]/40 pb-2 sm:pb-3 shrink-0">
                <div class="min-w-0">
                    <h2 class="text-sm sm:text-base font-extrabold text-[#2e2552] tracking-wide truncate">Kalender Rinci
                        Mingguan</h2>
                    <p class="text-[9.5px] sm:text-xs text-[#5a508f] mt-0.5 hidden sm:block">Jam kerja resmi (07:15 -
                        {{ now()->isFriday() ? '15:15' : '15:30' }} WIB)</p>
                </div>
                ...
                <div class="flex-1 min-w-0 flex flex-col relative h-full overflow-x-auto overflow-y-hidden">
                    <div class="min-w-[540px] sm:min-w-full flex-1 flex flex-col h-full min-h-[500px] sm:min-h-[560px]">
                        ...
                        <div class="grid grid-cols-8 border-b border-[#d4d1f5]/40 pb-1.5 sm:pb-2 relative z-0 shrink-0">
                            <!-- Time axes column -->
                            <div
                                class="text-center text-[10px] sm:text-xs font-bold text-[#5a508f] flex items-center justify-center">
                                Waktu</div>
                            <!-- 7 days columns -->
                            @foreach($dates as $date)
                                @php
                                ...
                                <div class="text-center flex flex-col items-center justify-center">
                                    <span
                                        class="text-[8.5px] sm:text-[9px] uppercase font-bold {{ $isSunday ? 'text-rose-500 font-extrabold' : ($isSaturday ? 'text-indigo-500 font-extrabold' : 'text-[#8e88dd]') }}">{{ $date->translatedFormat('D') }}</span>
                                    <span
                                        class="text-[10px] sm:text-xs font-bold mt-0.5 px-2 py-0.5 rounded-lg sm:rounded-xl transition-all duration-200 
                                    {{ $isDateToday ? 'bg-[#2e2552] text-white shadow-sm' : ($isDateSelected ? 'bg-[#8e88dd]/20 text-[#2e2552]' : ($isSunday ? 'text-rose-600 font-black' : ($isSaturday ? 'text-indigo-600 font-black' : 'text-[#5a508f]'))) }}">
                                        {{ $date->day }}
                                    </span>
                                </div>
                        ...
                        <!-- Grid container with time axes rows & events overlay -->
                        <div
                            class="min-h-[500px] sm:min-h-[560px] flex-1 grid grid-cols-8 relative z-10 select-none border-b border-[#d4d1f5]/40">
                            @php
                                $labelTimes = [
                                    '07:15' => ['top' => 0.0, 'transform' => 'translate(-50%, 2px)'],
                                    '08:00' => ['top' => 9.09, 'transform' => 'translate(-50%, -30%)'],
                            ...
                            <div class="border-r border-[#d4d1f5]/40 h-full relative z-10 select-none pointer-events-none">
                                @foreach($labelTimes as $timeStr => $item)
                                    <span
                                        class="absolute left-1/2 flex items-center justify-center bg-white px-1 z-20 text-[10px] font-extrabold text-[#5a508f] pointer-events-none"
                                        style="top: {{ number_format($item['top'], 2, '.', '') }}%; transform: {{ $item['transform'] }};">
                                        {{ $timeStr }}
                                    </span>
                                @endforeach
                            </div>
                            ...
                            @foreach($dates as $date)
                                ...
                                <!-- Column relative container -->
                                <div
                                    class="h-full border-r border-[#d4d1f5]/40 last:border-0 relative {{ ($isSunday || $isSaturday) ? 'bg-rose-50/40' : 'bg-[#fcfbff]' }} group/col">
                                    ...
                                    @foreach($events as $event)
                                        ...
                                        @if($event->has_access)
                                            ...
                                            <a href="{{ route('agenda.show', $event->id) }}"
                                                ...
                                                <div class="min-w-0 w-full overflow-hidden shrink-0">
                                                    <div
                                                        class="flex items-center justify-between text-[8px] font-bold opacity-90 gap-1 uppercase min-w-0">
                                                        <span class="whitespace-nowrap shrink-0">{{ substr($event->jam_mulai, 0, 5) }} -
                                                            {{ substr($event->jam_selesai, 0, 5) }}</span>
                                                        <span
                                                            class="px-1 py-0.5 rounded bg-black/10 text-[8px] font-semibold truncate">{{ $event->singkatan_bidang }}</span>
                                                    </div>
                                                    <h4 class="text-[10px] font-bold mt-0.5 leading-tight line-clamp-2 break-all">
                                                        {{ $event->judul }}</h4>
                                                </div>
                                                <div
                                                    class="text-[8px] opacity-90 truncate flex items-center gap-0.5 font-semibold w-full overflow-hidden shrink-0 mt-0.5">
                                                ...
                                                <!-- Floating Tooltip on Hover -->
                                                <div
                                                    class="absolute {{ $tooltipPosition }} {{ $arrowClass }} {{ $tStyle['bg'] }} {{ $tStyle['border'] }} {{ $tStyle['text'] }} w-56 sm:w-60 p-3 rounded-2xl shadow-2xl z-50 text-[10px] pointer-events-none opacity-0 group-hover:opacity-100 transition-all duration-200 border after:content-[''] after:absolute after:border-4 after:border-transparent">
                                                    <div
                                                        class="font-bold border-b border-black/10 pb-1 flex justify-between items-start gap-2">
                                                        <span
                                                            class="{{ $tStyle['header_text'] }} font-extrabold uppercase leading-tight mr-2">{{ $event->singkatan_bidang }}</span>
                                                        <span
                                                            class="{{ $tStyle['subtext'] }} whitespace-nowrap shrink-0 text-right mt-0.5">{{ substr($event->jam_mulai, 0, 5) }}
                                                            - {{ substr($event->jam_selesai, 0, 5) }}</span>
                                                    </div>
                                                    <div class="mt-1.5 font-bold leading-tight">
                                                        {{ $event->judul }}
                                        ...
                                        @else
                                            <div class="absolute p-2 border rounded-2xl text-left shadow-sm z-10 overflow-hidden {{ $cardColorClass }}"
                                                ...
                                                style="top: calc({{ number_format($topPct, 2, '.', '') }}% + 2px); height: calc({{ number_format($heightPct, 2, '.', '') }}% - 4px); left: calc({{ number_format($leftPos, 2, '.', '') }}% + 2px); width: calc({{ number_format($colWidth, 2, '.', '') }}% - 4px);">
                                                <div
                                                    class="flex items-center justify-between text-[8px] font-bold opacity-60 gap-1 uppercase min-w-0">
                                                    <span class="whitespace-nowrap shrink-0">{{ substr($event->jam_mulai, 0, 5) }} -
                                                        {{ substr($event->jam_selesai, 0, 5) }}</span>
                                                    <span
                                                        class="px-1 py-0.5 rounded bg-black/5 truncate">{{ $event->singkatan_bidang }}</span>
                                                </div>
                                                <h4
                                                    class="text-[10px] font-bold text-slate-500 mt-1 italic flex items-center gap-1 min-w-0 overflow-hidden">
                            ...
                            <div class="absolute inset-0 pointer-events-none opacity-40 z-0">
                                @foreach($labelTimes as $timeStr => $topPct)
                                    @if($topPct > 0 && $topPct <= 100)
                                        <div class="absolute w-full border-b border-[#d4d1f5]/30 h-0"
                                            style="top: {{ number_format($topPct, 2, '.', '') }}%;"></div>
                                    @endif
                                @endforeach
                            </div>
                        </div>
        ...
        <div x-show="openAddModal" x-cloak
            ...
            <div @click.away="openAddModal = false"
                ...
                <!-- Modal Header -->
                <div
                    class="px-5 py-3.5 sm:py-4 bg-gradient-to-r from-[#09103c] via-[#1b3bbb] to-[#09103c] text-white flex items-center justify-between shrink-0">
                    ...
                    <button @click="openAddModal = false" type="button"
                        class="p-1.5 bg-white/10 hover:bg-rose-500/80 rounded-xl text-white transition-all cursor-pointer shrink-0">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <form action="{{ route('agenda.store') }}" method="POST"
                    class="p-4 sm:p-5 space-y-2.5 flex-1 min-h-0 overflow-y-auto">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        <div class="sm:col-span-2 space-y-1">
                            <label for="judul"
                                class="block text-[11px] font-bold text-[#5a508f] uppercase tracking-wider">Judul Kegiatan /
                                Rapat <span class="text-rose-500 font-bold">*</span></label>
                            <input type="text" name="judul" id="judul" required
                                placeholder="Contoh: Rapat Koordinasi Layanan SPBE"
                                class="w-full px-3.5 py-2 bg-[#f8f7ff] hover:bg-[#f3f2fe] border border-[#d4d1f5] rounded-xl text-[#2e2552] text-xs placeholder-slate-400 focus:bg-white focus:border-[#1b3bbb] focus:ring-2 focus:ring-[#1b3bbb]/20 transition-all font-semibold">
                        </div>
                        <div class="space-y-1">
                            <label for="kategori"
                                class="block text-[11px] font-bold text-[#5a508f] uppercase tracking-wider">Kategori <span
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        <div class="col-span-2 sm:col-span-1 space-y-1">
                            <label for="tanggal"
                                class="block text-[11px] font-bold text-[#5a508f] uppercase tracking-wider">Tanggal <span
                                    class="text-rose-500 font-bold">*</span></label>
                            <input type="date" name="tanggal" id="tanggal" required x-model="selectedDate"
                                min="{{ now()->subMonths(6)->toDateString() }}"
                                max="{{ now()->addMonths(6)->toDateString() }}"
                        ...
                        <div class="space-y-1">
                            <label for="jam_mulai"
                                class="block text-[11px] font-bold text-[#5a508f] uppercase tracking-wider">Jam Mulai <span
                                    class="text-rose-500 font-bold">*</span></label>
                            <input type="time" name="jam_mulai" id="jam_mulai" required x-model="selectedTime"
                                class="w-full px-2.5 py-2 bg-[#f8f7ff] hover:bg-[#f3f2fe] border border-[#d4d1f5] rounded-xl text-[#2e2552] text-xs font-semibold focus:bg-white focus:border-[#1b3bbb] focus:ring-2 focus:ring-[#1b3bbb]/20 transition-all">
                        </div>
                        <div class="space-y-1">
                            <label for="jam_selesai"
                                class="block text-[11px] font-bold text-[#5a508f] uppercase tracking-wider">Jam Selesai
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div class="space-y-1">
                            <label for="tempat"
                                class="block text-[11px] font-bold text-[#5a508f] uppercase tracking-wider">Tempat / Ruangan
                                <span class="text-rose-500 font-bold">*</span></label>
                            <div class="relative">
                                <select id="tempat" name="lokasi" required
                                    class="w-full pl-3.5 pr-8 py-2 bg-[#f8f7ff] hover:bg-[#f3f2fe] border border-[#d4d1f5] rounded-xl text-[#2e2552] text-xs font-semibold appearance-none focus:bg-white focus:border-[#1b3bbb] focus:ring-2 focus:ring-[#1b3bbb]/20 transition-all cursor-pointer">
                        ...
                        <div class="space-y-1">
                            <label for="deskripsi"
                                class="block text-[11px] font-bold text-[#5a508f] uppercase tracking-wider">Deskripsi
                                (Opsional)</label>
                            <input type="text" name="deskripsi" id="deskripsi"
                                placeholder="Masukkan rincian singkat agenda..."
                                class="w-full px-3.5 py-2 bg-[#f8f7ff] hover:bg-[#f3f2fe] border border-[#d4d1f5] rounded-xl text-[#2e2552] text-xs placeholder-slate-400 focus:bg-white focus:border-[#1b3bbb] focus:ring-2 focus:ring-[#1b3bbb]/20 transition-all font-semibold">
                        </div>
                    </div>
                    ...
                    <div x-data='{
                        ...
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-[10.5px] text-slate-500 font-medium">Pilih bidang dan kelola anggota yang
                                    wajib hadir</p>
                            </div>
                        ...
                        <div class="bg-[#f8f7ff] border border-[#d4d1f5] rounded-2xl p-3 space-y-2">
                            @if(Auth::user()->isSekretarisMaster())
                                <label
                                    class="flex items-center gap-2 px-2.5 py-1.5 bg-white rounded-xl border border-[#d4d1f5] hover:border-[#1b3bbb] transition-all cursor-pointer select-none">
                                    ...
                                    <span class="text-xs text-[#2e2552] font-extrabold">Semua Bidang / Rapat Lintas Dinas (Semua
                                        Orang)</span>
                                </label>
                            @endif
                            ...
                            <div class="grid grid-cols-1 gap-1 max-h-[140px] overflow-y-auto pr-1">
                                ...
                                <label
                                    class="flex items-center justify-between px-2.5 py-1.5 rounded-xl border border-transparent hover:border-[#d4d1f5] hover:bg-white transition-all cursor-pointer select-none">
                                    <div class="flex items-center gap-2">
                                        <input type="checkbox" value="kadin" x-model="kadinTarget"
                                            @change="toggleKadinTarget()"
                                ...
                                @foreach($bidangs as $bid)
                                    ...
                                    @endphp
                                    <label
                                        class="flex items-center justify-between px-2.5 py-1.5 rounded-xl border border-transparent hover:border-[#d4d1f5] hover:bg-white transition-all cursor-pointer select-none {{ $isSubbag ? 'pl-6' : '' }}">
                                        <div class="flex items-center gap-2">
                                            @if($isSubbag)
                                                <span class="text-[#8e88dd] text-xs font-bold shrink-0 -mr-1">└</span>
                                            @endif
                                            <input type="checkbox" value="{{ $bid->id }}" x-model="bidangs"
                                                @change="checkBidang('{{ $bid->id }}')" @if($isUserBidang) disabled @endif
                                                class="w-4 h-4 rounded border-[#d4d1f5] text-[#1b3bbb] focus:ring-[#1b3bbb] focus:ring-offset-0 transition-all shrink-0">
                                            <span
                                                class="text-xs text-[#2e2552] font-semibold {{ $isUserBidang ? 'font-extrabold text-[#1b3bbb]' : '' }}">
                                                {{ $bid->nama }} <span
                                                    class="text-[#5a508f] font-normal">({{ $bid->singkatan }})</span>
                                            </span>
                                        </div>
                                        @if($isUserBidang)
                                            <span
                                                class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-extrabold bg-amber-50 text-amber-700 border border-amber-200/80 shrink-0 ml-2">
                                                Wajib Hadir
                                            </span>
                                        @endif
                                    </label>
                        ...
                        <div x-show="participantModalOpen" x-cloak
                            ...
                            <div @click.away="participantModalOpen = false"
                                ...
                                <div
                                    class="px-5 py-4 bg-gradient-to-r from-[#09103c] via-[#1b3bbb] to-[#09103c] text-white flex items-center justify-between shrink-0">
                                    <div class="flex items-center gap-3">
                                        <div class="p-2 bg-white/10 rounded-xl border border-white/15 shrink-0">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-[11px] text-indigo-100 font-medium">Cari nama & centang anggota
                                                bidang yang diundang</p>
                                        </div>
                                    </div>
                                    <button @click="participantModalOpen = false" type="button"
                                        class="p-1.5 bg-white/10 hover:bg-rose-500/80 rounded-xl text-white transition-all cursor-pointer shrink-0">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <div class="p-4 sm:p-5 overflow-y-auto no-scrollbar space-y-4 flex-1 bg-slate-50/50">
                                    <!-- Search Bar -->
                                    <div class="relative">
                                        <input type="text" x-model="searchParticipant"
                                            placeholder="Cari nama, NIP, atau jabatan peserta..."
                                        ...
                                        <svg class="w-4 h-4 text-[#1b3bbb] absolute left-3 top-2.5" fill="none"
                                            stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path>
                                        </svg>
                                        <button type="button" x-show="searchParticipant.length > 0"
                                    ...
                                    <template x-if="bidangs.length === 0">
                                        <div
                                            class="p-8 text-center bg-white rounded-2xl border border-dashed border-[#d4d1f5] shadow-2xs">
                                            <p class="text-xs text-slate-500 font-bold">Pilih minimal satu bidang di atas
                                                terlebih dahulu untuk mengelola peserta.</p>
                                        </div>
                                    </template>

                                    <!-- Group Card Khusus Kepala Dinas (Kadin) -->
                                    <template
                                        x-if="kadinUser && kadinTarget && (!searchParticipant || kadinUser.name.toLowerCase().includes(searchParticipant.toLowerCase()) || kadinUser.jabatan.toLowerCase().includes(searchParticipant.toLowerCase()))">
                                        <div
                                            class="bg-white border border-[#d4d1f5]/80 rounded-2xl p-3.5 space-y-2.5 shadow-2xs">
                                            <div
                                                class="flex items-center justify-between pb-2 border-b border-[#d4d1f5]/40">
                                                <div class="flex items-center gap-2">
                                                    <span class="w-2.5 h-2.5 rounded-full bg-[#1b3bbb]"></span>
                                                    <span class="text-xs font-extrabold text-[#2e2552]">Kepala Dinas
                                                        (Kadin)</span>
                                                </div>
                                                <button type="button" @click="toggleKadinTarget()"
                                                    class="text-[11px] font-extrabold text-[#1b3bbb] hover:underline cursor-pointer">
                                                    <span
                                    ...
                                    <template x-for="bidang in visibleBidangs" :key="bidang.id">
                                        <div
                                            ...
                                            <div
                                                ...
                                                <div class="flex items-center gap-2">
                                                    <span class="w-2.5 h-2.5 rounded-full bg-[#1b3bbb]"></span>
                                                    <span class="text-xs font-extrabold text-[#2e2552]"
                                                        x-text="bidang.nama + ' (' + bidang.singkatan + ')'"></span>
                                                </div>
                                                <button type="button" @click="toggleBidangUsers(bidang.id)"
                                                    class="text-[11px] font-extrabold text-[#1b3bbb] hover:underline cursor-pointer">
                                                    <span
                                                        x-text="isBidangAllChecked(bidang.id) ? 'Hapus Centang Semua' : 'Centang Semua'"></span>
                                                </button>
                                            </div>
                                                ...
                                                <template x-for="user in filteredUsers(bidang.users)" :key="user.id">
                                                    <label
                                                        ...
                                                        <input type="checkbox" :value="user.id"
                                                            x-model="selectedParticipants" @change="isDirty = true"
                                ...
                                <div
                                    class="px-5 py-3.5 bg-[#f8f7ff] border-t border-[#d4d1f5] flex flex-col sm:flex-row items-center justify-between gap-3 shrink-0">
                                    <div class="text-xs font-bold text-[#5a508f] flex items-center gap-1">
                                        <template x-if="selectedParticipants.length === 0">
                                            <span class="text-rose-600 font-black flex items-center gap-1">⚠️ Pilih minimal
                                                1 peserta!</span>
                                        </template>
                                        <template x-if="selectedParticipants.length > 0">
                                            <span>Total Terpilih: <span class="text-[#1b3bbb] font-extrabold"
                                                    x-text="selectedParticipants.length"></span> Peserta</span>
                                        </template>
                                    </div>
                                    <div class="flex items-center gap-2 w-full sm:w-auto justify-end">
                                        <button type="button" @click="participantModalOpen = false"
                                            class="px-4 py-2 bg-slate-200 hover:bg-slate-300 text-[#2e2552] text-xs font-bold rounded-xl transition-all cursor-pointer">
                                            Tutup
                                        </button>
                                        <button type="button"
                    ...
                    <div class="flex items-center justify-between gap-2 border-t border-[#d4d1f5]/60 pt-3 shrink-0">
                        <div
                            class="flex items-center justify-between p-2 px-3 bg-gradient-to-r from-[#f8f7ff] to-[#f3f2fe] border border-[#d4d1f5] rounded-xl flex-1 min-w-0">
                            <div class="flex items-center gap-1.5 min-w-0">
                                <svg class="w-3.5 h-3.5 text-[#1b3bbb] shrink-0" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                </svg>
                                <label for="butuh_presensi"
                                    class="text-[11px] sm:text-xs font-bold text-[#2e2552] cursor-pointer whitespace-nowrap truncate">Memerlukan
                                    Presensi Digital?</label>
                            </div>
                            <label class="relative inline-flex items-center cursor-pointer select-none ml-1.5 shrink-0"
                                x-data="{ checked: true }">
                                <input type="checkbox" name="butuh_presensi" id="butuh_presensi" :checked="checked"
                                ...
                                <div @click="checked = !checked"
                                    ...
                                    <div :style="checked ? 'transform: translateX(16px) !important; background-color: #ffffff !important;' : 'transform: translateX(0px) !important; background-color: #ffffff !important;'"
                                        class="w-4 h-4 rounded-full shadow-md transition-transform duration-200 border border-slate-200">
                                    </div>
                                </div>
                            </label>
                        </div>
                        ...
                        <div class="flex items-center justify-end gap-1.5 shrink-0">
                            ...
                            <button type="submit"
                                ...
                                <svg class="w-3.5 h-3.5 text-indigo-200 shrink-0" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
