...
<body class="min-h-screen md:h-screen md:overflow-hidden text-[#2e2552] bg-[#eef2ff] antialiased app-batik-bg">
    ...
    <div x-data="{ mobileNavOpen: false, profileMenuOpen: false }" class="min-h-screen md:h-screen flex flex-col p-2.5 sm:p-3.5 md:p-4 pt-2 sm:pt-2.5 md:pt-3 gap-4.5 sm:gap-5.5 md:gap-6 max-w-[1600px] w-full mx-auto md:overflow-hidden bg-[#eef2ff] app-batik-bg">
        ...
        <div id="mobile-nav-drawer" 
            ...
            @if(Auth::check())
                <div class="pt-2 border-t border-slate-200/80 mt-1">
                    <form action="{{ route('logout') }}" method="POST" class="w-full">
                        @csrf
                        <button type="submit" class="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-xs font-bold text-rose-600 bg-rose-50 hover:bg-rose-100 border border-rose-100 transition-colors shadow-xs">
        ...
        <div class="flex-1 min-h-0 min-w-0 flex flex-col md:flex-row gap-3.5 sm:gap-4 md:gap-4.5 items-stretch">
            ...
            <aside class="hidden md:flex w-60 bg-white rounded-3xl border border-slate-200/60 flex-col py-6 shrink-0 z-20 shadow-md text-[#09103c]">
            ...
            <main class="flex-1 min-w-0 bg-white rounded-xl sm:rounded-2xl md:rounded-[28px] shadow-md relative overflow-hidden flex flex-col text-[#090c24] border border-[#d4d1f5]/60">
                <div id="main-content" class="flex-1 min-h-0 w-full overflow-y-auto no-scrollbar p-3.5 sm:p-5 md:p-6 flex flex-col gap-3.5 sm:gap-5 justify-between">
                    ...
                    <div id="pjax-container" class="flex-1 min-w-0 w-full flex flex-col justify-between">
                        <div class="flex-1 min-w-0 w-full flex flex-col pb-3 sm:pb-4">
                            @yield('content')
                        </div>

                        <!-- Footer line placed neatly below content -->
                        <footer class="border-t border-[#d4d1f5]/60 py-3.5 mt-6 sm:mt-8 text-center text-slate-400 text-[10px] sm:text-xs font-semibold tracking-wider w-full shrink-0 flex items-center justify-center">
