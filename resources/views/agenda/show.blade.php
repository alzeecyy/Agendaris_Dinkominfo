...
<div x-data="{
            ...
            <div x-data="{ 
                ...
                <div x-show="openEditModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/50 backdrop-blur-sm">
                    <div @click.away="openEditModal = false" class="bg-white border border-[#d4d1f5]/60 rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden relative">
                        ...
                        <form action="{{ route('agenda.update', $agenda->id) }}" method="POST" class="p-6 space-y-4 max-h-[75vh] overflow-y-auto no-scrollbar text-[#2e2552]">
                            @csrf
                            @method('PUT')
                                                      ...
                                                      <div class="bg-slate-50 border border-slate-200/80 rounded-2xl p-3.5 space-y-2.5">
                                                          <div class="flex items-center justify-between pb-2 border-b border-slate-200/60">
                                                              <div class="flex items-center gap-2">
                                                                  ...
                                                                  <span class="text-xs font-black text-[#09103c]">Kepala Dinas (Kadin)</span>
                                                              </div>
                                                          ...
                                                          <div class="grid grid-cols-1 sm:grid-cols-2 gap-1.5">
                                                              <label class="flex items-start gap-2.5 p-2 bg-white rounded-xl border border-slate-200/60 hover:border-indigo-200 cursor-pointer select-none transition-all">
                                                                  <input type="checkbox" :value="kadinUser.id" x-model="selectedParticipants" @change="if(kadinUserId) kadinTarget = selectedParticipants.map(String).includes(String(kadinUserId));" class="w-4 h-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 mt-0.5 shrink-0">
                                                                  <div class="min-w-0 flex-1">
                                                                      <div class="text-xs font-bold text-slate-800 leading-tight truncate" x-text="kadinUser.name"></div>
                                                                      <div class="text-[10px] text-slate-500 font-medium truncate" x-text="kadinUser.jabatan || 'Kepala Dinas / Kadin'"></div>
                                                                  </div>
                                                              </label>
                                                          </div>
                                                      </div>
    ...
    @if($agenda->butuh_presensi && ($isSecretaryOfAgenda || Auth::user()->isSekretarisMaster() || Auth::user()->isSekretarisBidang()))
        <div class="bg-white border border-[#d4d1f5]/60 rounded-xl md:rounded-[32px] p-2.5 sm:p-6 shadow-sm space-y-2.5 sm:space-y-4">
            ...
            <div class="max-h-[460px] overflow-y-auto no-scrollbar pr-1">
    ...
    <div x-show="openDetailModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/50 backdrop-blur-sm">
        <div @click.away="openDetailModal = false" class="bg-white border border-[#d4d1f5]/60 rounded-3xl w-full max-w-2xl shadow-2xl overflow-hidden relative text-[#2e2552]">
            ...
            <div class="p-6 max-h-[60vh] overflow-y-auto no-scrollbar">
